...
setTimeout(() => closeModal("success", { force: true }), SUCCESS_CLOSE_DELAY_MS);
  ...
  function getStoredMegaskaSessionToken() {
    try {
  ...
  function getStoredCartToken() {
    ...
    for (const key of storageKeys) {
      try {
        const token = sanitizeCartToken(window?.localStorage?.getItem(key));
        if (token) return token;
      } catch {}
    }
    return "";
  ...
  async function fetchActiveCartContextFromAjax() {
  ...
  function readCartContext() {
    return fetchActiveCartContextFromAjax()
      .then((ctx) => {
        const cartToken = sanitizeCartToken(ctx?.cartToken || getStoredCartToken() || "");
      ...
      .catch(() => {
        const cartToken = sanitizeCartToken(getStoredCartToken() || "");
        const cartId = cartToken.startsWith("gid://shopify/Cart/") ? cartToken : "";
        return {
          cartToken,
          cartId,
  ...
  function buildWalletMountTarget() {
    const selectors = [
      "[data-cart-footer]",
      "[data-cart-summary]",
      ".cart__footer",
  ...
  function renderWalletCartUi(summary) {
    if (!summary || document.querySelector("[data-megaska-wallet-ui='true']")) return;
    const availableToRedeem = Number(summary?.wallet?.availableToRedeem || 0);
    const mountTarget = buildWalletMountTarget();
    if (!mountTarget) return;

    const wrapper = document.createElement("section");
    wrapper.setAttribute("data-megaska-wallet-ui", "true");
    ...
    wrapper.innerHTML = `
      <h3 style="margin:0 0 8px 0;">Wallet Balance</h3>
      <p style="margin:0 0 10px 0;font-weight:600;">${formatInrFromMinor(availableToRedeem)}</p>
      <button type="button" data-megaska-wallet-apply="1" ${availableToRedeem > 0 ? "" : "disabled"}>
        ${availableToRedeem > 0 ? "Apply Wallet" : "No Wallet Balance"}
      </button>
    `;

    const button = wrapper.querySelector("[data-megaska-wallet-apply='1']");
    if (button) {
      button.addEventListener("click", () => {
        applyWalletFromCartButton(button, availableToRedeem);
      });
    }

    mountTarget.prepend(wrapper);
    console.log("[WALLET CART UI] mounted", {
      availableToRedeem,
      mounted: true,
    });
  }

  async function initCartWalletUi() {
    const path = String(window?.location?.pathname || "");
    if (!/^\/cart(?:\/|$|\?)/.test(path)) return;

    try {
      const summary = await fetchDashboardSummaryForCartWallet();
      renderWalletCartUi(summary);
    } catch (error) {
      console.error("[WALLET CART UI] apply failed", {
        error: error instanceof Error ? error.message : String(error),
      });
