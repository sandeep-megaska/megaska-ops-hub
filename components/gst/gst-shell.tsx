  ...
  if (!enabled) {
    return (
      <div className="mx-auto max-w-5xl p-6">
        ...
        <p className="mt-2 text-sm text-gray-600">Enable NEXT_PUBLIC_ENABLE_GST_UI to access test-only GST screens.</p>
      </div>
    )
  }
      ...
      <div className="rounded-2xl border p-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">Test/admin GST console with backend-controlled flows only.</p>
          </div>
          <div className="rounded-full border px-3 py-1 text-xs font-medium uppercase tracking-wide">Mode: {mode}</div>
        </div>
      </div>

      <nav className="flex flex-wrap gap-2">
        {nav.map((item) => (
          <Link key={item.href} href={item.href} className={`rounded-xl border px-4 py-2 text-sm ${pathname === item.href ? 'font-semibold' : ''}`}>
            {item.label}
          </Link>
        ))}
      </nav>
