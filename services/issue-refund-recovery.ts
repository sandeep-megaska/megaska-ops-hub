...
export async function resolveIssueRefundSnapshot(issueId: string, input: ResolveIssueRefundSnapshotInput = {}): Promise<RecoveryResult> {
  ...
  if (!issue.shopId || !issue.shopifyOrderId) {
    await prisma.orderActionRequest.update({
      where: { id: issue.id },
      data: { shopId: issue.shopId || resolvedShopId, shopifyOrderId: issue.shopifyOrderId || resolvedOrderId },
    });
  }
  ...
  console.info("[ISSUE REFUND RECOVERY] resolved", {
    issueId: issue.id,
    resolvedShopId,
    resolvedOrderId,
    resolvedAmount,
...
function logSkip(issueId: string, input: Omit<RecoveryResult, "ok">): RecoveryResult {
  console.warn("[ISSUE REFUND RECOVERY] skip", {
    issueId,
    resolvedShopId: input.resolvedShopId || null,
    resolvedOrderId: input.resolvedOrderId || null,
    resolvedAmount: input.resolvedAmount || null,
    resolvedPaymentMethod: input.resolvedPaymentMethod || null,
    refundRequestId: input.refundRequestId || null,
    walletTransactionId: null,
    skipReason: input.error,
  });
